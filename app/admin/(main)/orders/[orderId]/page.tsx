...
export default async function OrderPage({ params }: Props) {
                        ...
                        {order.status === "PENDING" && (
                            <>
                                <form
                                    action={async () => {
                                        "use server";
                                        await updateOrderStatus(order._id, "APPROVE");
                                    }}
                                >
                                    <button
                                        className="
    ...
    "
                ...
                {/* AI ANALYSIS */}
                <div className="mb-6">
                    <h2 className="font-medium mb-2">AI Fraud Analysis</h2>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="p-4 border rounded">
                            ...
                            <p className="font-bold">
                                {(order.aiAnalyst.probability * 100).toFixed(2)}%
                            </p>
                        </div>
              ...
              ${order.aiAnalyst.risk === "LOW" && "bg-green-100 text-green-600"}
              ${order.aiAnalyst.risk === "MEDIUM" && "bg-yellow-100 text-yellow-600"}
              ${order.aiAnalyst.risk === "HIGH" && "bg-red-100 text-red-600"}
            `}
                            >
            {order.aiAnalyst.risk}
          </span>
                        </div>
                    </div>
                </div>
